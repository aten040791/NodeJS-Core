...
Với api response đã được format thông qua `response.ok()`: Xem chi tiết ở `modules/auth/controllers/authController.js` hàm `helloWorld()`. Các api response của controllers
đối với api response not found sẽ được format thông qua `responseUtil.[tên_method](res)`. Với `responseUtils`, xem thêm tại `services/responseUtils.js`
